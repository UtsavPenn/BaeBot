...
def get_power_player_for_user(user_id):
    player = get_squad_details(user_id)['powerPlayer']
    return get_player(player).name


def get_power_players():
    return [(get_league_team_name_for_user(user),
             get_power_player_for_user(user)) for user in USER_IDS]


def power_players(bot, update, args):
    if args:
