...
from ipl_fantasy.common import (USER_IDS, 
...
def get_sub_strike_rates():
    sub_strike_rates = [(get_league_team_name_for_user(user),
                        get_sub_strike_rate_for_user(user)) for user in USER_IDS]
...
def sub_strike_rate(bot, update, args):
    if args:
        ...
        bot.send_message(update.message.chat_id, get_sub_strike_rate_for_user(user))
    else:
        bot.send_message(update.message.chat_id, simple_table(get_sub_strike_rates()))
